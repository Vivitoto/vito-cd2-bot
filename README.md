...
| 📋 查询菜单 | 发送 `/tasks` 查看未完成任务，发送 `/routes` 查看当前路由配置摘要 |
...
容器启动时会自动尝试初始化企微应用菜单。默认包含三个一级入口：

| 菜单 | 事件 key | 作用 |
|---|---|---|
| 查询 → 任务状态 | `status` | 查看未完成中转任务 |
| 查询 → 路由配置 | `routes` | 查看当前默认路由、中转目录和各路由路径摘要 |
...
2. 添加一级菜单 **查询**，下面添加点击事件子菜单 **任务状态** / **路由配置**
3. 子菜单事件 key 分别填 `status` / `routes`
4. 一级菜单 **使用说明** / **健康检查** 类型选 **点击事件**，事件 key 分别填 `help` / `health`
5. 保存
